	...
	echo '<form method="post" id="'.$criteria['form']['name'].'" name="'.$criteria['form']['name'].'">';
		...
		for($i=0; $i<count($fields); $i++) {
			...
			elseif($fields[$i]['type']=='password') {
				echo '<label>'.$fields[$i]['title'].'</label>';
				echo '<p><input '.$fields[$i]['class'].' type="password" id="'.$fields[$i]['name'].'" name="'.$fields[$i]['name'].'" style="width:100%" value="'.$fields[$i]['value'].'"></p>';
			}
			else {
				echo '<label>'.$fields[$i]['title'].'</label>';
				echo '<p><input '.$fields[$i]['class'].' type="text" id="'.$fields[$i]['name'].'" name="'.$fields[$i]['name'].'" style="width:100%" value="'.$fields[$i]['value'].'"></p>';
			}
		}
		
		echo '<p>';
		echo '<input type="submit" class="btn default" id="'.$submit['name'].'" name="'.$submit['name'].'" value="'.$submit['value'].'">';
		echo '</p>';
